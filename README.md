...
7. [CreateCountryUseCase](create-in-transaction/usecases.py#L8) (пользовательский кейс 
использует два репозитория и одну сессию, чтобы выполнить сохранения экземпляров двух моделей 
в транзакции). 
...
### Пользовательский сценарий с двумя репозиториями и одной сессией

В папке [create-in-transaction](create-in-transaction) реализован пользовательский сценарий,
который использует два репозитория, которые используют одну и ту же сессию для того, чтобы 
выполнить сохранение данных в транзакции - так удастся либо сохранить все, либо не сохранить ничего.
